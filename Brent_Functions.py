...
def process_brent_dataset(input_folder):
    all_files = os.listdir(input_folder) # Scanning files
    brent_files = [f for f in all_files if f.startswith('BRN') and f.endswith('.csv')] # Only BRN csv files
    ...
    for filename in brent_files:
        ...
        df = DF.filter_trading_hours(df, start_time='06:00', end_time='22:00')
...
def run_brent_signature_analysis(input_folder):
    freqs = ['1s', '5s', '15s', '30s', '1min', '2min', '5min', '10min', '20min']
    freq_seconds = [1, 5, 15, 30, 60, 120, 300, 600, 1200]
    
    vol_data = {f: [] for f in freqs}
    all_files = sorted([f for f in os.listdir(input_folder) if f.startswith('BRN') and f.endswith('.csv')])
    ...
    for i, filename in enumerate(all_files):
        ...
        df = DF.filter_brent_expiry(df, filename)
        df = DF.filter_trading_hours(df, start_time='06:00', end_time='22:00')
        df_1s = DF.apply_median_aggregation_doll(df, freq='1s')
