...
def process_wbs_dataset(input_folder):
    all_files = os.listdir(input_folder)
    wbs_files = [f for f in all_files if f.startswith('WBS') and f.endswith('.csv')]
...
def run_wti_signature_analysis(input_folder):
    freqs = ['1s', '5s', '15s', '30s', '1min', '2min', '5min', '10min', '20min']
    freq_seconds = [1, 5, 15, 30, 60, 120, 300, 600, 1200]
    
    vol_data = {f: [] for f in freqs}
    all_files = sorted([f for f in os.listdir(input_folder) if f.startswith('WBS') and f.endswith('.csv')])
