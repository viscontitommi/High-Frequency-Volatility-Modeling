...
def process_tfm_dataset(input_folder):
    all_files = os.listdir(input_folder)
    tfm_files = [f for f in all_files if f.startswith('TFM') and f.endswith('.csv')]
    
    rv_results = []
    for filename in tfm_files:
...
def run_tfm_signature_analysis(input_folder):
    freqs = ['1s', '5s', '15s', '30s', '1min', '2min', '5min', '10min', '20min']
    freq_seconds = [1, 5, 15, 30, 60, 120, 300, 600, 1200]
    
    vol_data = {f: [] for f in freqs}
    all_files = sorted([f for f in os.listdir(input_folder) if f.startswith('TFM') and f.endswith('.csv')])
